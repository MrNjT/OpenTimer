...
class Edge {
  ...
  public:
    ...
    inline edge_type_e type() const;                        // Query the egde type.
    ...
    inline void_t set_type(edge_type_ce);                   // Set the edge type.
    ...
    timing_pt timing_ptr(int, int, int);                    // Query the timing ptr.
...
// Procedure: set_type
inline void_t Edge::set_type(edge_type_ce edge_type) {
...
inline void_t Edge::set_delay(int el, int irf, int orf, float_ct delay) {
  __numeric_guard(_delay[el][irf][orf] = delay);
}
...
// Function: type
// Return the type.
inline edge_type_e Edge::type() const {
  return _edge_type;
}
...
inline void_t Jump::set_timing_sense(timing_sense_ce sense) {
  CHECK(sense == POSITIVE_UNATE || sense == NEGATIVE_UNATE);
  _timing_sense = sense;
}
