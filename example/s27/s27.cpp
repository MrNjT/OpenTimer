...
int main(int argc, char *argv[]) {
  ...
  timer.set_verilog_fpath("s27.v");
  timer.set_early_celllib_fpath("s27_Early.lib");
  timer.set_late_celllib_fpath("s27_Late.lib");
  timer.set_timing_fpath("s27.timing");
  timer.set_spef_fpath("s27.spef");
  ...
  for(const auto&t : timer.circuit_ptr()->testset()) {
    EL_RF_ITER(el, rf) {
      printf("N-RAT: %s/%.4f\n", t->constrained_pin_ptr()->name().c_str(), t->constrained_pin_ptr()->node_ptr()->rat(el, rf));
      printf("T-RAT: %s/%.4f\n", t->constrained_pin_ptr()->name().c_str(), t->rat(el, rf));
    }
  }
  ...
  timer.disconnect_pin("inst_3:ZN");
  timer.connect_pin("inst_3:ZN", "TAUNET_1");
  timer.connect_pin("TAUGATE_1:A", "TAUNET_1");
  timer.connect_pin("TAUGATE_1:Z", "net_14");
  timer.read_spef("change_1.spef");

  // Example: Query the timing for a pin at any transition and split.
  printf("LATE/FALL slack at pin 'G17' = %.4f\n", timer.slack("G17", LATE, FALL)); 
  printf("LATE/RISE arrival time at pin 'G17' = %.4f\n", timer.at("G17", LATE, RISE)); 
  printf("EALRY/FALL required arrival time at pin 'G17' = %.4f\n", timer.rat("G17", EARLY, FALL)); 
